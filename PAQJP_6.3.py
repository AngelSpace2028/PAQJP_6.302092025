...
class PAQJPCompressor:
    ...
    def transform_14(self, data, repeat=255):
        """Transform data by analyzing '01' patterns, reversing '0101' sequences, repeating until 32 bytes or max 255 iterations."""
        if not data:
        ...
        logging.info(f"transform_14: Max {max_cycles} cycles, {repeat} repeats for {len(data)} bytes")
        
        output_bits = []
        current_binary = binary_str
        iteration_count = 0
        
        while iteration_count < max_cycles:
            temp_bits = []
            i = 0
            while i < len(current_binary) - 1:
                if current_binary[i:i+2] == '01':
                    if i + 4 <= len(current_binary):
                        segment = current_binary[i:i+4]
                        if segment == '0101':
                            # Reverse the 4-bit sequence
                            temp_bits.extend(segment[::-1])  # 0101 -> 1010
                            temp_bits.extend(['0', '0', '1'])  # Append padding "001"
                        else:
                            # Reorder: third, fourth, first, second
                            third, fourth, first, second = segment[2], segment[3], segment[0], segment[1]
                            temp_bits.extend([third, fourth, first, second])
                            temp_bits.extend(['1', '0', '1'])  # Append padding "101"
                        i += 4
                    else:
                        # Handle remaining bits
                        temp_bits.extend(current_binary[i:])
                        i = len(current_binary)
                else:
                    temp_bits.append(current_binary[i])
                    i += 1
            current_binary = ''.join(temp_bits)
            output_bits = temp_bits
            iteration_count += 1
            
            # Check if output is at least 32 bytes
            bit_str = ''.join(current_binary)
            byte_length = (len(bit_str) + 7) // 8
            if byte_length >= 32:
                logging.info(f"transform_14: Reached {byte_length} bytes after {iteration_count} iterations")
                break
        
        # Convert back to bytes (base-256)
        bit_str = ''.join(current_binary)
        byte_length = (len(bit_str) + 7) // 8
    ...
    def reverse_transform_14(self, data, repeat=255):
        """Reverse transform_14 by decoding padding and restoring bit sequences."""
        if len(data) < 1:
        ...
        binary_str = bin(int.from_bytes(data, 'big'))[2:].zfill(len(data) * 8)
        data_size_kb = len(data) / 1024
        max_cycles = min(iteration_count, max(1, int(data_size_kb)))  # Use stored iteration count
        logging.info(f"reverse_transform_14: {max_cycles} cycles, {repeat} repeats for {len(data)} bytes")
        ...
        for _ in range(max_cycles):
            temp_bits = []
            i = 0
            while i < len(current_binary):
                if i + 7 <= len(current_binary):
                    segment = current_binary[i:i+4]
                    padding = current_binary[i+4:i+7]
                    if padding == '001' and segment == '1010':
                        # Restore reversed "0101" (1010) to "0101"
                        temp_bits.extend(['0', '1', '0', '1'])
                        i += 7
                    elif padding == '101':
                        # Restore reordered sequence to original (first, second, third, fourth)
                        third, fourth, first, second = segment
                        temp_bits.extend([first, second, third, fourth])
                        i += 7
                    else:
                        temp_bits.append(current_binary[i])
                        i += 1
